...
## Weekly verification — 2026-08-22

FSMA's authorised-Belgian-CASP list was last verified 2026-08-14 and showed no Belgian-authorised CASPs in that specific list. This does not exclude EEA-passported providers. Kraken's licensing page continues to state Irish MiCA CASP authorisations passported across the EEA.

Kraken's current official fee page contains multiple spot schedules/sections:
...
Therefore the app must resolve:
`applicable_fee = current account + pair + product + fee program`
and persist the fee schedule used. Do not hardcode one website rate.

Kraken's current EEA restriction list remains extensive and must be refreshed dynamically.
