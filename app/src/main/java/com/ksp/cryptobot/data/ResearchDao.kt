...
interface ResearchDao {
    ...
    @Query("SELECT * FROM research_state ORDER BY updatedAtEpochMs ASC")
    suspend fun allState(): List<ResearchStateEntity>

    @Query("DELETE FROM research_events")
    suspend fun clearEvents()

    @Query("DELETE FROM research_strategy_profiles")
    suspend fun clearProfiles()

    @Query("DELETE FROM research_state")
