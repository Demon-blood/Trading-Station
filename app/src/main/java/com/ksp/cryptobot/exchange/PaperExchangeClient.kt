...
class PaperExchangeClient(
    ...
    /**
     * PAPER mode should simulate orders with fake money, but the market data should still be
     * live. When running inside the Android app we therefore delegate public market-data calls
     * to Kraken's unauthenticated public REST endpoints. No API key/secret is used here.
     *
     * When this class is used as a no-context fallback by live connectors, live delegation is
    ...
    override suspend fun getTicker(symbol: String): MarketTicker {
        krakenPublicMarketData?.let { live ->
            return runCatching { live.getTicker(symbol) }.getOrElse {
                simulatedTicker(symbol)
            }
        }
    ...
    override suspend fun validateSymbol(symbol: String): ExchangeSymbolInfo {
        return krakenPublicMarketData?.let { live ->
            runCatching { live.validateSymbol(symbol) }.getOrNull()
